...
class Dataset_Physio(Dataset):
    ...
    def _resolve_paths(self):
        pt_paths = {
            'train': os.path.join(self.root_path, 'train.pt'),
            'val': os.path.join(self.root_path, 'val.pt'),
            'test': os.path.join(self.root_path, 'test.pt'),
        }
        pkl_paths = {
            'train': os.path.join(self.root_path, 'samples_train.pkl'),
            'val': os.path.join(self.root_path, 'samples_val.pkl'),
            'test': os.path.join(self.root_path, 'samples_test.pkl'),
        }

        has_pt = any(os.path.exists(p) for p in pt_paths.values())
        has_pkl = any(os.path.exists(p) for p in pkl_paths.values())
        if has_pt:
            return 'pt', pt_paths
        if has_pkl:
            return 'pkl', pkl_paths

        raise FileNotFoundError(
            "No supported HAR/EEG dataset files found in {}. "
            "Expected train/val/test .pt or samples_{split}.pkl files.".format(self.root_path)
        )

    def _select_split_path(self, paths, split):
        path = paths.get(split)
        if path and os.path.exists(path):
            return path
        if split == 'val':
            fallback = paths.get('test')
            if fallback and os.path.exists(fallback):
                return fallback
        if split == 'test':
            fallback = paths.get('val')
            if fallback and os.path.exists(fallback):
                return fallback
        raise FileNotFoundError(
            "Missing {} split for dataset in {}. Looked for {}.".format(
                split, self.root_path, path
            )
        )

    def _maybe_transpose_batch(self, samples):
        if torch.is_tensor(samples) and samples.ndim == 3:
            if samples.shape[1] < samples.shape[2]:
                return samples.transpose(1, 2)
        if isinstance(samples, np.ndarray) and samples.ndim == 3:
            if samples.shape[1] < samples.shape[2]:
                return samples.transpose(0, 2, 1)
        return samples

    def _maybe_transpose_sample(self, sample):
    ...
    def __read_data__(self):
        mode, paths = self._resolve_paths()
        split = self.flag
        data_path = self._select_split_path(paths, split)

        if mode == 'pt':
            data = self._torch_load(data_path)
            if isinstance(data, dict):
                samples = data.get('samples', data.get('data'))
                labels = data.get('labels', data.get('label'))
                if samples is None or labels is None:
                    raise ValueError(
                        "Expected keys 'samples'/'labels' in {} but found {}.".format(
                            data_path, list(data.keys())
                        )
                    )
            elif isinstance(data, (list, tuple)) and len(data) == 2:
                samples, labels = data
            else:
                raise ValueError("Unsupported .pt format at {}".format(data_path))

            samples = self._maybe_transpose_batch(samples)
            if torch.is_tensor(samples):
                self.data_x = samples.float()
            else:
                self.data_x = torch.tensor(samples, dtype=torch.float32)

            if torch.is_tensor(labels):
                self.data_y = labels.long()
            else:
                self.data_y = torch.tensor(labels, dtype=torch.long)
            ...
            for entry in samples:
                if isinstance(entry, (list, tuple)) and len(entry) == 3:
                    ...
                    raise ValueError("Unexpected sample format in {}".format(data_path))

                data = self._maybe_transpose_sample(data)
                datas.append(data)
                labels.append(label)

            try:
                stacked = np.stack(datas, axis=0)
                stacked = self._maybe_transpose_batch(stacked)
                self.data_x = torch.tensor(stacked, dtype=torch.float32)
            except ValueError:
                self.data_x = [torch.tensor(d, dtype=torch.float32) for d in datas]

            self.data_y = torch.tensor(labels, dtype=torch.long)

        if torch.is_tensor(self.data_y):
            unique_labels = torch.unique(self.data_y).numel()
        else:
            unique_labels = len(set(self.data_y))
        print("Loaded {} samples from {} ({} classes)".format(len(self), data_path, unique_labels))
    ...
    def __getitem__(self, item):
        ...
        data_y = self.data_y[item]

        if not torch.is_tensor(data_x):
            data_x = torch.tensor(data_x, dtype=torch.float32)
        if not torch.is_tensor(data_y):
            data_y = torch.tensor(data_y, dtype=torch.long)

        x_mark = torch.zeros_like(data_x)
        y_mark = torch.zeros_like(data_y)
        return data_x, data_y, x_mark, y_mark
