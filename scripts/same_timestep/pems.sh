...
for pems in $pems_data; do
    ...
    for pred_len in $pred_lens; do
        python -u arctandiff_finetune_forecast.py \
            ...
            --epoch_to_load $epoch_to_load \
            --start_lr 0.0005 \
            --csv_suffix ${loss_type}_patch_size_${patch_size}_hidden_size_${hidden_size}_recon_head_${recon_head_depth}_${timestep_sampling}_lr${lr}_${diffusion_loss_type}_sample_same_timesteps_random_init \
            --base_model_lr_scale 1.0 \
            --save_test_metrics_csv
    done
done
...
# PEMS with instance normalization
...
for pems in $pems_data; do
    ...
    python -u arctandiff_train_diffusion_only.py \
        ...
        --model ArcTanDiffusion \
        --data "${pems}" \
        --save_dir "arctandiffusion_${pems}_${loss_type}_patch_size_${patch_size}_hidden_size_${hidden_size}_recon_head_${recon_head_depth}_${timestep_sampling}_lr${lr}_${diffusion_loss_type}_sample_same_timesteps_instance_norm" \
        --features M \
        --input_len 96 \
        --label_len 0 \
        --pred_len 0 \
    ...
    for pred_len in $pred_lens; do
        python -u arctandiff_finetune_forecast.py \
            ...
            --model ArcTanDiffusion \
            --data "${pems}" \
            --load_dir "arctandiffusion_${pems}_${loss_type}_patch_size_${patch_size}_hidden_size_${hidden_size}_recon_head_${recon_head_depth}_${timestep_sampling}_lr${lr}_${diffusion_loss_type}_sample_same_timesteps_instance_norm" \
            --features M \
            --input_len 96 \
            --label_len 48 \
            --pred_len $pred_len \
            ...
            --epoch_to_load $epoch_to_load \
            --start_lr 0.0005 \
            --csv_suffix ${loss_type}_patch_size_${patch_size}_hidden_size_${hidden_size}_recon_head_${recon_head_depth}_${timestep_sampling}_lr${lr}_${diffusion_loss_type}_sample_same_timesteps_instance_norm \
            --base_model_lr_scale 1.0 \
            --save_test_metrics_csv
    done
done
