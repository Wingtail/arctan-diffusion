...
summary_csv="${out_dir}/EEG_classification_cps_robustness_summary.csv"
...
for loss_type in $loss_types; do
  echo "=== Robustness test for EEG loss_type: ${loss_type} ==="
  ...
  if [ ! -f "$combined_ckpt" ]; then
    echo "Combined EEG checkpoint not found for ${loss_type}."
