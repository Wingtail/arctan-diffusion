...
summary_csv="${out_dir}/Epilepsy_classification_cps_robustness_summary.csv"
...
for loss_type in $loss_types; do
  echo "=== Robustness test for Epilepsy loss_type: ${loss_type} ==="
  ...
  pretrain_dir_default="arctandiffusion_epilepsy_${loss_type}_patch_size_${patch_size}_hidden_size_${hidden_size}_recon_head_${recon_head_depth}_stripped_lr${lr}_${diffusion_loss_type}"
  ...
  if [ -n "$pretrain_override" ]; then
    ...
    pretrain_dir="$(find . -maxdepth 2 -type d \
      -name "arctandiffusion_epilepsy_${loss_type}_patch_size_${patch_size}_hidden_size_${hidden_size}_recon_head_${recon_head_depth}_stripped_lr${lr}_${diffusion_loss_type}*" \
      2>/dev/null | head -n 1)"
  fi
  ...
  if [ ! -f "$combined_ckpt" ]; then
    echo "Combined Epilepsy checkpoint not found for ${loss_type}."
