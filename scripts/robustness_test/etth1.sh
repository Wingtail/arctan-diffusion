...
# Mirrors scripts/ablations/ETTh1.sh (same-timestep loss ablation)
epoch_to_load=50
...
summary_csv="${out_dir}/ETTh1_loss_robustness_summary.csv"
...
for loss_type in $loss_types; do
  ...
  pretrain_dir_default="arctandiffusion_${src_dataset}_${loss_type}_patch_size_${patch_size}_hidden_size_${hidden_size}_recon_head_${recon_head_depth}_${timestep_sampling}_lr${lr}_${diffusion_loss_type}_same_timesteps"
  pretrain_dir_alt="arctandiffusion_${src_dataset}_${loss_type}_patch_size_${patch_size}_hidden_size_${hidden_size}_recon_head_${recon_head_depth}_${timestep_sampling}_lr${lr}_${diffusion_loss_type}_sample_same_timesteps"
  if [ -n "$pretrain_override" ]; then
    pretrain_dir="$pretrain_override"
  elif [ -d "$pretrain_dir_default" ]; then
    pretrain_dir="$pretrain_dir_default"
  elif [ -d "$pretrain_dir_alt" ]; then
    pretrain_dir="$pretrain_dir_alt"
  else
    pretrain_dir="$(find . -maxdepth 2 -type d \( \
      -name "${pretrain_dir_default}_*" -o \
      -name "${pretrain_dir_alt}_*" \
      \) 2>/dev/null | head -n 1)"
  fi

  if [ -z "${pretrain_dir:-}" ] || [ ! -d "$pretrain_dir" ]; then
    echo "Pretrain directory not found for ${loss_type}."
    echo "Expected: ${pretrain_dir_default} (or ${pretrain_dir_alt})"
    echo "Tip: set ${pretrain_env}=/path/to/pretrain_dir"
    exit 1
  fi
  ...
  if [ ! -f "$combined_ckpt" ]; then
    ...
    echo "Tip: set ${ckpt_env}=/path/to/checkpoint"
    exit 1
  fi
