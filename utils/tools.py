...
def _load_state_dict(weights_path, device="cpu"):
...
def save_full_checkpoint(path, filename, model, args=None, epoch=None, optimizer=None, extra=None):
...
def transfer_weights(weights_path, model, exclude_head=True, device='cpu'):
    new_state_dict = _load_state_dict(weights_path, device=device)
