...
parser.add_argument(
    "--skip_pretrain",
    action="store_true",
    help="skip loading pretrain checkpoints during finetuning",
)
parser.add_argument(
    "--linear_probe",
    action="store_true",
    help="train only the finetune head and save best head by validation loss",
)
parser.add_argument(
...
parser.add_argument(
    "--linear_probe_head_path",
    type=str,
    default=None,
    help="path to a saved linear-probe head checkpoint (loaded before testing)",
)
...
if args.task_name == "pretrain":
    ...
    for ii in range(args.itr):
        ...
        if args.load_checkpoints is None and not args.skip_pretrain:
